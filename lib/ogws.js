...
const authConfig = new ApiV1Auth();
const authExpiresIn = 3153600;
...
//     this.config = {
//       apiUrl: 'https://ogws.orbcomm.com/api/v1.0/',
//       auth: {
//         expiresIn: 3153600,
//         accessId: '',
//         password: '',
...
//     /**
//    * Send a request to get an access_token.
...
//     const auth_params = {
//       client_id: this.config.auth.accessId,
//       client_secret: this.config.auth.password,
//       expires_in: this.config.auth.expiresIn,
//       grant_type: 'client_credentials',
//     }
//     const response = await mgsApiPost_(endpoint, auth_params, 'application/x-www-form-urlencoded');
//     logger.debug("Authentication Token", response);
...
function mgsApiPost_(endpoint, data, contentType='application/json') {
  let postData ="";
...
async function getMgsVersion(auth) {
  ...
  return await mgsApiGet_(endpoint, auth);
}
...
async function getMgsTime(auth) {
  if (arguments) { overrideUrl_(arguments); }
  const endpoint = 'info/service';
  const response = await mgsApiGet_(endpoint, auth);
  if(response){
...
async function getForwardStatuses(auth, filter) {
  ...
  if (response.Statuses) {
    ...
    logger.debug(`${messagesStatusesCount} statuses retrieved from Mailbox ` +
        auth.accessId);
  }
if (overrideRemap_(arguments)) {
    return response;
