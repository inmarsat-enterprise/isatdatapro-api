...
let apiHost = 'api.inmarsat.com';
let apiBasePath = '/v1/idp/gateway/rest/';
let apiUrl = 'https://' + apiHost + apiBasePath;
...
function overrideUrl_(args) {
  for (let a=0; a < args.length; a++) {
    if ((typeof(args[a]) === 'string') && args[a].startsWith('http')) {
      ...
      apiHost = reqUrl.hostname;
      apiBasePath = reqUrl.pathname;
    }
  }
}
 ...
 */
function mgsApiGet_(endpoint, auth, filters) {
  return new Promise((resolve, reject) => {
    let reqUrl = new URL(endpoint, 'https://' + apiHost + apiBasePath);
    if (auth) {
      reqUrl.searchParams.append('access_id', auth.accessId);
      reqUrl.searchParams.append('password', auth.password);
    }
    if (filters) {
    ...
    https.get(options, (res) => {
      ...
      throw new Error(handleRequestError_(err, null, endpoint));
    });
  });
}

function mgsApiPost_(endpoint, data) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(options, (res) => {
      if (res.statusCode < 200 || res.statusCode > 299) {
        ...
        res.on('timeout', () => { reject('Request timed out'); });
      }
    });
    req.on('error', (err) => {
      throw new Error(handleRequestError_(err, null, endpoint));
...
async function getMgsVersion() {
  ...
  const endpoint = 'info_version.json';
  return await mgsApiGet_(endpoint);
}
 ...
 */
async function getMgsTime() {
  if (arguments) { overrideUrl_(arguments); }
  const endpoint = 'info_utc_time.json';
  const idpTimeUtc = await mgsApiGet_(endpoint);
...
async function getErrorDefinitions() {
  if (arguments) { overrideUrl_(arguments); }
  const endpoint = 'info_errors.json';
  let response = await mgsApiGet_(endpoint);
...
async function getReturnMessages(auth, filter) {
  if (arguments) { overrideUrl_(arguments); }
  const endpoint = 'get_return_messages.json';
  let apiFilter = {
  ...
  let response = await mgsApiGet_(endpoint, auth, apiFilter);
  if (response.Messages) {
    const msgCount = response.Messages !== null ? response.Messages.length : 0;
    const more = response.More ? ' more messages from ' : '';
    logger.debug(`${msgCount} messages retrieved from Mailbox ${auth.accessId}` +
        ` after ${filter.startTimeUtc}` + more +
        ` next high watermark ${response.NextStartUTC}`);
  }
  if (overrideRemap_(arguments)) {
    return response;
  }
...
async function getForwardMessages(auth, ids) {
  if (arguments) { overrideUrl_(arguments); }
  const endpoint = 'get_forward_messages.json';
  const apiFilter = { fwIDs: getForwardIdsString_(ids) };
  let response = await mgsApiGet_(endpoint, auth, apiFilter);
  if (overrideRemap_(arguments)) {
    return response;
  }
...
async function getForwardStatuses(auth, filter) {
  if (arguments) { overrideUrl_(arguments); }
  const endpoint = 'get_forward_statuses.json';
  let apiFilter = {};
  ...
  if (response.Statuses) {
    const messagesStatusesCount = response.Statuses !== null ? response.Statuses.length : 0;
...
async function cancelForwardMessages(auth, ids) {
  if (arguments) { overrideUrl_(arguments); }
  const endpoint = 'submit_cancelations.json';
  let apiFilter = { fwIDs: getForwardIdsString_(ids) };
  let response = await mgsApiGet_(endpoint, auth, apiFilter);
  if (overrideRemap_(arguments)) {
    return response;
  }
...
async function getMobileIds(auth, filter) {
  if (arguments) { overrideUrl_(arguments); }
  const endpoint = 'get_mobiles_paged.json';
  const apiFilter = {
    page_size: 1000,
  };
  if (filter) {
...
async function getBroadcastIds(auth) {
  ...
  let response = await mgsApiGet_(endpoint, auth);
  if (overrideRemap_(arguments)) {
    return response;
  }
  return remapJsonKeys_(response, templates.MOBILE_OR_BROADCAST_INFO);
