...
const idpmsgPath = './test/codecs/fieldedge-iotdemo.idpmsg';
...
describe('#importCodec()', function() {
  ...
  it('should import idpmsg (xml)', function() {
    let converted = importCodec(idpmsgPath);
...
describe('#String Field', function() {
  ...
  it('should decode a string field', function() {
    const testVal = '{"key1": "value1"}';
    ({buffer, offset} = encodeField(testField, testVal, buffer, offset));
    ({decoded, offset} = decodeField(testField, buffer, 0));
    expect(decoded.value).to.equal(testVal);
  });
...
const tcTextMo = {
  ...
  isMo: true,
  idpmsgFile: '',
  encoded: [255, 4, 84, 158, 201, 217, 154, 89, 91, 25, 9, 206, 136, 9, 208,
            73, 203, 8, 9, 217, 24, 93, 25, 73, 206, 136, 9, 204, 140, 12, 141,
            11, 76, 12, 75, 76, 12, 72, 12, 76, 142, 140, 12, 14, 140, 12, 9,
            203, 8, 9, 216, 218, 25, 88, 218, 217, 89, 9, 206, 136, 21, 28, 157,
            89, 75, 8, 9, 221, 25, 91, 92, 25, 88, 93, 29, 92, 153, 73, 206,
            136, 12, 140, 11, 140, 159, 64],
  decoded: {
    name: 'textMobileOriginated',
    serviceKey: 255,
    messageKey: 4,
    fields: {
      text: "{'field': 'A', 'date': '2024-01-01 12:00:00', 'checked': True, 'tempeature': 20.2}",
    },
  },
};
const testMessages = [
...
describe('#encodeMessage()', function() {
  it('should encode the message', function() {
    const testMessage = require('./codecs/testMobileTerminated.json');
    const encoded = encodeMessage(testMessage, modemCodecPath);
    expect(Buffer.isBuffer(encoded)).to.be.true;
    expect(encoded).to.have.length(3);
  });
});
